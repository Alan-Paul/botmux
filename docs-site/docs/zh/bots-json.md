...
| `groupDefaultModels` | 按群 ID 配置新话题默认模型，例如 `{ "oc_team": { "codex": "your-codex-model", "claude-code": "sonnet" } }`；目前仅支持 Codex 和 Claude。可在 Dashboard「群管理 → 新话题默认模型」按 Bot 配置 |
...
每个 Bot 的 `groupDefaultModels` 独立配置；不同群、不同 Bot 的模型互不影响。模型名称支持自定义，必须是目标 CLI/服务商可用的名称。
...
优先级：显式触发模型 > 新话题保存的群模型 > 同 CLI 的 Bot 模型 > 原有 CLI 不匹配回退。此配置不改变 CLI 类型、运行环境或思考强度。

Dashboard 保存后无需重启 daemon。清空某个输入框可取消该 CLI 的群级覆盖；两个都清空会删除该群配置。手动编辑 `bots.json` 则沿用原有配置加载方式。
