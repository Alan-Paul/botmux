...
| `groupDefaultModels` | Per-chat defaults for new topics, e.g. `{ "oc_team": { "codex": "your-codex-model", "claude-code": "sonnet" } }`. Supports Codex and Claude; configure each bot from Dashboard group management |
...
Each bot owns its own `groupDefaultModels`, keyed by chat ID and CLI. Custom model names must be supported by the selected CLI/provider. A new topic captures these settings when its session is created; later edits or clearing the group configuration do not alter that topic on restart or resume. Selecting a CLI uses only its matching entry. Topics without a group override retain the existing live bot-model fallback. Direct messages, chat-scoped group sessions, and adopted external sessions do not use the snapshot.

Precedence: explicit trigger model > captured group model > matching bot model > existing cross-CLI fallback. This does not change the CLI, runtime, or reasoning effort. Dashboard saves apply without restarting the daemon. Empty fields remove overrides for future topics; manual file edits follow the existing configuration-loading procedure.
